$(document).ready(function() {
   ...
   function sygzclb(data){
			...
			for(var i = 0; i < bb.length; i++) {
				...
				var batime3='';
				if(!!bb[i]&&!!bb[i].ubDto&&!!bb[i].ubDto.startTime){
					
					//获取两次的值
					var timeb=bb[i].ubDto.startTime;
					var timee=bb[i].ubDto.endTime;
					//转化一下格式
				...
				var collectionNum='';
				if(!!bb[i] && bb[i].hasOwnProperty('ubDto') && bb[i].ubDto.hasOwnProperty('collectionNum')){
					collectionNum=bb[i].ubDto.collectionNum;
				}
				//bb[i].ubDto.disposeNum
				var disposeNum='';
				if(!!bb[i]&&bb[i].hasOwnProperty('ubDto')&&!!bb[i].ubDto.hasOwnProperty('disposeNum')){
				...
				$(".fixassetlistny").append(
					...
					'<a href="pcperson-bond-details.html?id='+asetid+'">' +
					...
					'<span class="lbazczqra-spa">' + debtType+ '</span>' +*/
					...
					'</p>' +
					'<span class="bajejj">' +
					    '<em>'+totalInterestMoney+'</em>'+
					    '<em>'+totalMoneyw+'</em>'+
					    '<em>'+assessTotalPrice+'</em>'+
					'</span>'+
					'</div>' +
					'<div class="lbazqnrbr">' +
