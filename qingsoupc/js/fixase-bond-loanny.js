$(document).ready(function() {
	...
	/*内页固定资产列表*/
	qsExec('sale', '/fixed/noVerify/fixedList', 'post', {}, false, true, sygzclb);
   function sygzclb(data){
   	        var bb =data.fixedAssetList?data.fixedAssetList:[];
   	        console.log('内页固定资产，逾期贷款，债券 列表',data);
			for(var i = 0; i <= bb.length; i++) {
				//传一个id给详情页
				var asetid ='';
				if(!!bb[i]&&!!bb[i].faDto&&!!bb[i].faDto.id){
					asetid=bb[i].faDto.id;
				}
				...
				$(".fixassetlistny").append(
					...
					'<a href="pcfix-assets-details.html?id='+asetid+'">' +
