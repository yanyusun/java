...
var editAssets = function(id, o){
	...
	qsExec('sale', '/fixed/noVerify/getDetail', 'get', {fixedAssetId: id}, false, true, function(res){
		var record = res.fixedAsset;		//资产包对象
		...
		$("#righterType").find('em').each(function(){
			if($(this).data('val')==record.righterType){
				$(this).addClass('radiuscurb');
			}else{
				$(this).removeClass('radiuscurb');
			}
		...
		$("input[name='fixedAsset.title']").val(record.title);
		$("#comfrom").val(record.comfrom);
		$("#entrust_start").val(record.entrustBegintime);
		$("#entrust_end").val(record.entrustEndtime);
		$("#assetRental").val(record.assetRental);
		...
		$("#houseNature").val(record.houseNature);
		$("#houseSize").val(record.houseSize);
		$("#floor").val(record.floor);
		$("#orientation").val(record.orientation);
		$("#hasHouseCertificate").val(record.hasHouseCertificate);
		$("#year").val(record.year);
		$("#isPledge").val(record.isPledge);
		$("#usedYear").val(record.usedYear);
		$("#isLaw").val(record.isLaw);
		$("#isJudgement").val(record.isJudgement);
		$("#assetDescribe").val(record.assetDescribe);
		$("#righterName").val(record.righterName);
		$("#righterProvince").val(record.righterProvince);
		$("#righterCity").val(record.righterCity);
		$("#righterAddress").val(record.righterAddress);
		$("#righterPhone").val(record.righterPhone);
		$("#righterContactName").val(record.righterContactName);
		$("#righterContactPhone").val(record.righterContactPhone);
	});
...
function getFixedList(params, listSelector, pageSelector){
	...
	qsExec('sale', '/fixed/list', 'get', params, true, true, function(res){
		...
		data.map(function(item, i){
			...
			content += '<span class="lzlglbtopsph">查看</span>';
...
$(function(){
    ...
    function autoRightProvinc(res){
    	$("#right_pca").find("select[name='fixedAsset.righterProvince']").html('');
    	$("#right_pca").find("select[name='fixedAsset.righterProvince']").append("<option value=''>请选择</option>");

    	res.map(function(o){
    		$("#right_pca").find("select[name='fixedAsset.righterProvince']").append("<option value='"+o.value+"'>"+o.label+"</option>");
		});
    }
    ...
    function autoRightCity(res){
    	$("select[name='fixedAsset.righterCity']").html('');
    	$("select[name='fixedAsset.righterCity']").append("<option value=''>请选择</option>");
    	if(res){
    		res.map(function(o){
				$("select[name='fixedAsset.righterCity']").append("<option value='"+o.value+"'>"+o.label+"</option>");
			});
    	}
    }
	...
	$(document).delegate("select[name='fixedAsset.righterProvince']", 'change', function(){
		var aid = $(this).val();
		if(!!aid){
			//加载市
	...
	$(document).delegate(".page-link", 'click', function(){
		...
		getFixedList(params, listSelector, pageSelector);
	})

	getFixedList({page:1, pageCount:qs_pagination.pageCount, businessStatus:1020 }, '#table_toBeAudit', '#toBeAudit');
	getFixedList({page:1, pageCount:qs_pagination.pageCount, businessStatus:1030}, '#table_retorted', '#retorted');
	getFixedList({page:1, pageCount:qs_pagination.pageCount, businessStatus:1040}, '#table_toBePub', '#toBePub');
	getFixedList({page:1, pageCount:qs_pagination.pageCount, businessStatus:1050}, '#table_hasPubed', '#hasPubed');
	getFixedList({page:1, pageCount:qs_pagination.pageCount, businessStatus:1060}, '#table_hasOff', '#hasOff');
	getFixedList({page:1, pageCount:qs_pagination.pageCount, businessStatus:1070}, '#table_useless', '#useless');
    ...
    $('#assetForm').validate({
		rules: {
			'fixedAsset.title': 'required',
			'fixedAsset.entrustBegintime': 'required',
			'fixedAsset.entrustEndtime': 'required',
			'fixedAsset.assetRental': 'required',
			'fixedAsset.righterName': 'required',
			'fixedAsset.righterContactName': 'required',
			'fixedAsset.righterContactPhone': 'required',
		},
		messages:{
			'fixedAsset.title': '请填写标题',
			'fixedAsset.entrustBegintime': '请设置委托开始时间',
			'fixedAsset.entrustEndtime': '请设置委托结束时间',
			'fixedAsset.assetRental': '资产总额不能为空',
			'fixedAsset.righterName': '请填写产权方姓名',
			'fixedAsset.righterContactName': '请填写产权方姓名',
			'fixedAsset.righterContactPhone': '请填写产权方姓名',
		},
		submitHandler: function(form){
	    	...
	    	//产权方类型
	    	var righterType = $("#righterType").find('.radiuscurb').data('val');
	    	var dispose = JSON.stringify(disposesArr);
	    	var formData = $("#assetForm").serialize();
	    	formData += '&fixedAsset.assetType='+assetType+'&fixedAsset.province='+province+'&fixedAsset.city='+city
	    		+'&fixedAsset.righterType='+righterType;
	    	...
	    	var actionUrl = '/fixed/addFixed';

	    	if(recordId!==-1){
	    		formData += '&fixedAsset.id='+ recordId;
	    		actionUrl = '/fixed/updateFixed';
	    	}
